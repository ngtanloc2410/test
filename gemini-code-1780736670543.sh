...
cat <<SYSCTL > /etc/sysctl.d/99-multi-vnic.conf
...
cat <<NETPLAN > /etc/netplan/60-secondary-vnic.yaml
network:
    version: 2
    ethernets:
        ens5:
...
chmod 600 /etc/netplan/60-secondary-vnic.yaml
netplan apply
...
echo ">> Đang chạy Container Traffmonetizer 1 (VNIC 1)..."
...
echo ">> Đang chạy Container Traffmonetizer 2 (VNIC 2)..."
